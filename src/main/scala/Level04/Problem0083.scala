...
object Problem0083 {
  ...
  case class Grid(matrix: Vector[Vector[Int]], nrRows: Int, nrCols: Int):
    ...
    def getNeighbors(cell: Cell): List[Cell] =
      val Cell(x, y) = cell
      val candidates: Iterator[Cell] = Iterator(Cell(x, y + 1), Cell(x, y - 1), Cell(x - 1, y), Cell(x + 1, y))
  ...
  def calcShortestPath(grid: Grid): Int =
    ...
    def loop(distances: Map[Cell, Double], processed: Set[Cell]): Int =
      if heap.isEmpty then distances(Cell(nrRows - 1, nrCols - 1)).toInt
      else
  ...
  def main(args: Array[String]): Unit =
    val lines: Iterator[String] = scala.io.Source.fromResource("input0083.txt").getLines()
